...
import lasagne
import theano
from models import resnet50, densenet
...
def train_model(dataset, n_hidden=50, batch_size=100, epochs=100, learning_rate=0.01, model='cnn', l2_ratio=1e-7):
    ...
    print('batch_size=',batch_size,'epochs=',epochs,'learning_rate=',learning_rate,'l2_ratio=',l2_ratio,'n_hidden=',n_hidden)
    if model=='cnn'  or model == 'resnet' or model=='cnn2' or model=='Droppcnn' or  model=='densenet':
        input_var = T.tensor4('x')
    else:
        input_var = T.matrix('x')
    target_var = T.ivector('y')
    if model == 'resnet':
        print('Using a Resnet based model...')
        net = resnet50.model(shape=n_in, num_classes=n_out)
    elif model == 'cnn':
        ...
        print('Using a Densenet based model...')
        net = densenet.build_densenet(input_shape=n_in, classes=n_out)
    else:
        print('Using a single layer softmax based model...')
        net = get_softmax_model(n_in, n_out)

    # net['input'].input_var = input_var
    if type(net).__name__ == 'dict':
